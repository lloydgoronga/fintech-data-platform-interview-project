...
def extract(source_engine, mongo_client):
    ...
    try:
        ...
        if merchant_collection.count_documents({}) == 0:
            ...
            merchant_data = [
                {'merchant_name': 'GreenLeaf Grocers', 'category': 'Groceries', 'country': 'USA'},
                {'merchant_name': 'The Daily Grind Coffee', 'category': 'Food & Beverage', 'country': 'USA'},
                {'merchant_name': 'TechSphere Electronics', 'category': 'Electronics', 'country': 'Canada'},
            ]
...
def transform(customers_df, transactions_df, merchants_df):
    ...
    logging.info("Created dim_customer.")

    # Create Dimension: dim_merchant
    dim_merchant = merchants_df[['merchant_name', 'category']].drop_duplicates().copy()
    dim_merchant.rename(columns={'merchant_name': 'merchant_name'}, inplace=True)
    logging.info("Created dim_merchant.")

    # Create Dimension: dim_date
    transactions_df['transaction_date'] = pd.to_datetime(transactions_df['transaction_date'])
    date_range = pd.date_range(start=transactions_df['transaction_date'].min(), end=transactions_df['transaction_date'].max(), freq='D')
    ...
    logging.info("Created fact_transactions base.")
...
def load(dim_customer, dim_merchant, dim_date, fact_transactions, dwh_engine):
    ...
    try:
        ...
        logging.info("Data warehouse schema created/recreated successfully.")

        # Load dimensions
        dim_customer.to_sql('dim_customer', dwh_engine, if_exists='append', index=False)
        dim_merchant.to_sql('dim_merchant', dwh_engine, if_exists='append', index=False)
        dim_date.to_sql('dim_date', dwh_engine, if_exists='append', index=False)
        logging.info("Dimension tables loaded.")

        # Create surrogate key lookups
        customer_keys = pd.read_sql("SELECT customer_key, customer_id FROM dim_customer", dwh_engine)
        merchant_keys = pd.read_sql("SELECT merchant_key, merchant_name FROM dim_merchant", dwh_engine)
        ...
        # Load fact table
        final_fact_df.to_sql('fact_transactions', dwh_engine, if_exists='append', index=False)
